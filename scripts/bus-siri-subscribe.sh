...
# Source VPN detection script if available
...
get_local_host() {
    ...
    # Priority 1: Use SIRI_HOST_IP from VPN detection script
    if [[ -n "$SIRI_HOST_IP" ]]; then
        ip="$SIRI_HOST_IP"
        if [[ "$USE_VPN_IP" == "true" ]]; then
            echo "🔐 Using VPN IP: $ip" >&2
        else
            echo "🌐 Using regular network IP: $ip" >&2
        fi
    else
        # Fallback: Always dynamically determine IP address
        ...
        # Try route-based detection if no VPN IP found
        if [[ -z "$ip" ]]; then
    ...
    echo "http://${ip}:${BUS_SIRI_PORT:-8082}"
}
